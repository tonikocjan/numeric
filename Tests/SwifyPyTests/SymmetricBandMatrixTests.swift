//
//  SymmetricBandMatrixTests.swift
//  SwifyPyTests
//
...
class SymmetricBandMatrixTests: XCTestCase {
  func test3x3DiagonalSubscript() {
    var matrix: SymmetricBandMatrix = [[1, 2, 3]]
  ...
  func test3x3Tridiagonal() {
    var matrix: SymmetricBandMatrix = [[1, 3, 5], [2, 4]]
    XCTAssertEqual(1, matrix[0, 0])
  ...
  func testIdentity() {
    let id = SymmetricBandMatrix<Double>.identity(4)
    for i in 0..<id.width {
      for j in 0..<id.width {
