 ...
 */
public struct Vector<T: MatrixScalar>: ExpressibleByArrayLiteral {
...
public extension Vector where T == Double {
   ...
   */
  static func random(of size: Int, in range: Range<T>) -> Self {
    var vec = Vector(size: size)
    for i in 0..<size {
      vec[i] = Double.random(in: range)
    }
    return vec
  }
}
...
extension Vector: CustomDebugStringConvertible where T: LosslessStringConvertible, T: CVarArg {
  public var debugDescription: String {
    "[" + map { String(format: "%0.25f", $0) }.joined(separator: ", ") + "]"
  }
}

// MARK: - CustomStringConvertible
extension Vector: CustomStringConvertible where T: LosslessStringConvertible, T: CVarArg {
  public var description: String { debugDescription }
}

public extension Vector {
  subscript(range: Range<Int>) -> Vector {
    get {
      assert(range.startIndex >= 0)
      assert(range.upperBound <= count)
      var vec = Vector<T>(size: range.count)
...
public func +<T: MatrixScalar>(_ v: Vector<T>, _ num: T) -> Vector<T> {
  v.map { $0 + num }
}
...
/// Add a constant to every element of the vector.
public func +<T: MatrixScalar>(_ num: T, _ v: Vector<T>) -> Vector<T> {
  v.map { $0 + num }
}
...
/// Divide a constant with every element of the vector.
public prefix func -<T: MatrixScalar>(_ v: Vector<T>) -> Vector<T> {
  -1 * v
}

/// Element-wise vector addition.
public func +<T: MatrixScalar>(_ v1: Vector<T>, _ v2: Vector<T>) -> Vector<T> {
...
/// Element-wise `pow`.
public func pow<T: BinaryFloatingPoint>(_ v: Vector<T>, _ p: Double) -> Vector<T> {
  v.map { T(pow(Double($0), p)) }
}

/// Element-wise `abs`.
public func abs<T: BinaryFloatingPoint>(_ v: Vector<T>) -> Vector<T> {
  v.map { T(abs(Double($0))) }
}

/// - - - - -

extension Zip2Sequence {
  public func map<T: MatrixScalar>(_ transform: ((Sequence1.Element, Sequence2.Element)) throws -> T) rethrows -> Vector<T> {
    .init(arrayLiteral: try map(transform))
  }
}

extension Collection {
  public func map<T: MatrixScalar>(_ transform: (Element) throws -> T) rethrows -> Vector<T> {
    .init(arrayLiteral: try map(transform))
  }
}
